...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  if (pgdir[PDX(va)] != (pte_t) NULL) {
    ...
    if (create) {
      struct PageInfo* newpage = page_alloc(1);  // allocate new page
      if (newpage == NULL) {
        return NULL;
      } else {
        newpage->pp_ref++; // increate ref
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  for (i = (uint32_t) ROUNDDOWN(va, PGSIZE); i < (uint32_t) ROUNDUP(va + len, PGSIZE); i += PGSIZE) {
    pt = pgdir_walk(env->env_pgdir, (void*)i, 0);
    if (!pt || (i >= ULIM) || !(*pt & PTE_P) || ((*pt & perm) != perm)) {
      if (i < (uint32_t)va) {
